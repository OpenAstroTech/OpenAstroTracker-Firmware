...
class TimerInterrupt
{
  ...
  public:
    ...
    // Interval (in ms) and duration (in milliseconds). Duration = 0 or not specified => run indefinitely
    template <typename TArg> bool attachInterruptInterval(float interval, void (*callback)(TArg), TArg params, unsigned long duration = 0)
    {
    ...
    if (ITimer1.getTimer() == 1)
    {
        if (countLocal != 0)
        {
            if (ITimer1.checkTimerDone())
            {
                ...
                Serial.println("T1 callback, _OCRValueRemaining = " + String(ITimer1.get_OCRValueRemaining())
...
ISR(TIMER3_COMPA_vect)
{
    ...
    if (ITimer3.getTimer() == 3)
    {
        if (countLocal != 0)
        {
            ...
            {
                #if (TIMER_INTERRUPT_DEBUG > 1)
                Serial.println("T3 callback, _OCRValueRemaining = " + String(ITimer3.get_OCRValueRemaining())
                               + ", millis = " + String(millis()));
                #endif
                ITimer3.callback();
...
ISR(TIMER4_COMPA_vect)
{
    ...
    if (ITimer4.getTimer() == 4)
    {
        if (countLocal != 0)
        {
            ...
            {
                #if (TIMER_INTERRUPT_DEBUG > 1)
                Serial.println("T4 callback, _OCRValueRemaining = " + String(ITimer4.get_OCRValueRemaining())
                               + ", millis = " + String(millis()));
                #endif
                ITimer4.callback();
...
ISR(TIMER5_COMPA_vect)
{
    ...
    if (ITimer5.getTimer() == 5)
    {
        if (countLocal != 0)
        {
            ...
            {
                #if (TIMER_INTERRUPT_DEBUG > 1)
                Serial.println("T5 callback, _OCRValueRemaining = " + String(ITimer5.get_OCRValueRemaining())
                               + ", millis = " + String(millis()));
                #endif
                ITimer5.callback();
